...
class UploadedFileSpec extends ObjectBehavior
{
    ...
    public function its_an_http_uploaded_file()
    {
        $this->shouldBeAnInstanceOf(UploadedFileInterface::class);
    }

    public function it_is_initializable_from_an_uploaded_php_file_array()
    {
        $this->shouldHaveType(UploadedFile::class);
    }

    public function it_has_a_stream_with_file_contents()
    {
        $this->getStream()->shouldBeAnInstanceOf(StreamInterface::class);
    }

    public function it_has_a_file_size()
    {
        $content = 'Hello world, form a test!';
        $this->getSize()->shouldBe(\strlen($content));
    }

    public function it_has_a_client_file_name()
    {
        $this->getClientFilename()->shouldBe('test.txt');
    }

    public function it_has_the_file_media_type()
    {
        $this->getClientMediaType()->shouldBe('plain/text');
    }

    public function it_can_move_an_uploaded_file_to_a_new_location()
    {
    ...
    public function it_cannot_move_to_unknown_locations()
    {
        $this->shouldThrow(\InvalidArgumentException::class)
            ->during('moveTo', ['some/where']);
    }

    public function it_throws_exception_for_unfinished_uploads()
    {
    ...
    public function it_throws_exception_on_move_failure()
    {
    ...
    public function it_throws_exception_on_consecutive_calls()
    {
        $this->moveTo(__DIR__.'/test.txt');
        $this->shouldThrow(\RuntimeException::class)
            ->during('moveTo', [__DIR__.'/test.txt']);
